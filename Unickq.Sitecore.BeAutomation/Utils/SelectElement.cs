...
namespace Unickq.Sitecore.BeAutomation.Utils
{
    public class SelectElement : IWrapsElement
    {
        ...
        {
            if (element == null) throw new ArgumentNullException("element", "element cannot be null");

            if (string.IsNullOrEmpty(element.TagName) ||
                string.Compare(element.TagName, "select", StringComparison.OrdinalIgnoreCase) != 0)
                throw new ArgumentNullException("select", element.TagName);

            this.element = element;

            // let check if it's a multiple
            var attribute = element.GetAttribute("multiple");
        ...
        /// </summary>
        public IList<IWebElement> Options => element.FindElements(By.TagName("option"));
        ...
        {
            if (text == null) throw new ArgumentNullException("text", "text must not be null");
            ...
            if (!partialMatch)
                ...
                options = element.FindElements(By.XPath(".//option[normalize-space(.) = " + EscapeQuotes(text) + "]"));
            else
                options = element.FindElements(
                    By.XPath(".//option[contains(normalize-space(.),  " + EscapeQuotes(text) + ")]"));
            ...
            {
                var substringWithoutSpace = GetLongestSubstringWithoutSpace(text);
                ...
                if (string.IsNullOrEmpty(substringWithoutSpace))
                    ...
                    candidates =
                        element.FindElements(
                            By.XPath(".//option[contains(., " + EscapeQuotes(substringWithoutSpace) + ")]"));
            ...
            if (!matched) throw new NoSuchElementException("Cannot locate element with text: " + text);
        }
        ...
        {
            var builder = new StringBuilder(".//option[@value = ");
            ...
            if (!matched) throw new NoSuchElementException("Cannot locate option with value: " + value);
        }
        ...
        {
            var match = index.ToString(CultureInfo.InvariantCulture);
        ...
        public void DeselectAll()
        {
            ...
            foreach (var option in Options) SetSelected(option, false);
        }
        ...
        public void DeselectByText(string text)
        {
            ...
            var matched = false;
            var builder = new StringBuilder(".//option[normalize-space(.) = ");
            ...
            if (!matched) throw new NoSuchElementException("Cannot locate option with text: " + text);
        }
        ...
        private static string EscapeQuotes(string toEscape)
        {
                ...
                toEscape.IndexOf("'", StringComparison.OrdinalIgnoreCase) > -1)
            {
                ...
                var substrings = new List<string>(toEscape.Split('\"'));
                ...
                var quoted = new StringBuilder("concat(");
